...
(() => {
  ...
  // ---------------------------------------------------------------------------
  // Word-level Strong's token rendering
  //
  // lexicons/verse-words.json maps verse_id → [{text, strongs?}].
  // It is generated by scripts/build-lexicons.js (run once after setup).
  // We lazy-load it once on init and cache in state.verseWords.
  // When the map is loaded, renderReader() re-renders to show word tokens.
  // ---------------------------------------------------------------------------

  async function loadVerseWords() {
    if (state.verseWords !== null) return; // already loaded or attempted
    state.verseWords = {}; // mark as attempted so we don't retry on every render
    try {
      const resp = await fetch("lexicons/verse-words.json");
      if (!resp.ok) return; // file not yet generated — prose view stays
      state.verseWords = await resp.json();
      renderReader(); // re-render now that word data is available
    } catch {
      // silently ignore — the app works fine without word tokens
    }
  }
  ...
  // Returns the verse_id for a given section + verseMarker object.
  function markerVerseId(section, marker) {
    const book = bookToVerseIdPrefix(section.book);
    const verseMatch = String(marker.verse || "").match(/\d+/);
    const verse = verseMatch ? verseMatch[0] : marker.verse;
    return `${book}.${marker.chapter}.${verse}`;
  }

  // True when at least one verse in the section has word token data loaded.
  function sectionHasWordTokens(section) {
    if (!state.verseWords || !Object.keys(state.verseWords).length) return false;
    return (section.verseMarkers || []).some(m => {
      const vid = markerVerseId(section, m);
      return Array.isArray(state.verseWords[vid]) && state.verseWords[vid].length > 0;
    });
  }

  // Renders a passage with word-level Strong's tokens when verse-words data is
  // available. Falls back to the prose renderer so behaviour is unchanged while
  // lexicons/verse-words.json hasn't been generated yet.
  function renderOriginalPassageWithTokens(section, withNotice = false) {
    ...
    const seen = new Set();
    const tokenisedVerses = (section.verseMarkers || []).map(marker => {
      const verseId = markerVerseId(section, marker);
      if (seen.has(verseId)) return "";
      seen.add(verseId);
      const words = state.verseWords[verseId];
      if (!Array.isArray(words) || words.length === 0) return "";
      const label = marker.label || `${marker.chapter}:${marker.verse}`;
      return renderTokenisedVerse(marker.anchor || verseId, verseId, label, words);
    }).join("");
    ...
    return `<section class="passage passage-with-tokens" id="${escapeAttr(section.id)}" data-section-id="${escapeAttr(section.id)}">
      ...
      <div class="passage-meta">${source}${tier}</div>
      ${notice}
      <div class="passage-body tokens-body" data-section-id="${escapeAttr(section.id)}">${tokenisedVerses}</div>
    </section>`;
  }

  function renderTokenisedVerse(anchor, verseId, label, words) {
    return `<p id="${escapeAttr(anchor)}" class="verse-with-tokens" data-verse-id="${escapeAttr(verseId)}">
      ...
      <span class="verse-tokens">${formatParagraphWithTokenizedWords(words)}</span>
    </p>`;
  }
