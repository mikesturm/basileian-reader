...
(() => {
  ...
  const els = {
    ...
    sourceWordBtn: document.getElementById("sourceWordBtn"),
  ...
  function init() {
    ...
    if (window.TranslationsModule && typeof TranslationsModule.init === "function") {
      Promise.resolve(TranslationsModule.init()).then(() => {
        ...
        if (state.activeTranslation !== "basileia") {
          switchTranslation(state.activeTranslation);
        }
      }).catch(error => {
    ...
    if ("serviceWorker" in navigator && location.protocol.startsWith("http")) {
      navigator.serviceWorker.register("sw.js?v=20260508-fix2").catch(() => {});
    }
  }
  ...
  function bindEvents() {
    ...
    els.translationSelect.addEventListener("change", () => {
      switchTranslation(els.translationSelect.value);
    });
    ...
    if (els.sourceWordBtn) {
      els.sourceWordBtn.addEventListener("click", openOriginalLookupFromSelection);
    }
  ...
  async function switchTranslation(translationId) {
    state.activeTranslation = translationId || "basileia";
    localStorage.setItem(STORAGE_TRANSLATION, state.activeTranslation);

    if (state.activeTranslation !== "basileia") {
      try {
        updateTranslationStatus("loading");
        if (window.TranslationsModule && typeof TranslationsModule.loadTranslation === "function") {
          await TranslationsModule.loadTranslation(state.activeTranslation);
        }
  ...
  function canonicalVerseId(bookName, chapter, verse) {
    const bookMap = {
      "Mark": "mark",
      "Matthew": "matthew",
      "Luke": "luke",
    ...
    const book = bookMap[bookName] || String(bookName || "").toLowerCase().replace(/[^a-z0-9]/g, "");
    return `${book}.${chapter}.${verse}`;
  }

  function getCachedTranslationText(verseId) {
    if (state.activeTranslation === "basileia") return null;
    if (!window.TranslationsModule || typeof TranslationsModule.getCachedVerseText !== "function") return null;
    return TranslationsModule.getCachedVerseText(verseId, state.activeTranslation);
  }
  ...
  function renderPassage(section) {
    if (state.activeTranslation !== "basileia") {
      const translated = renderTranslatedPassage(section);
  ...
  function renderTranslatedPassage(section) {
    const markers = section.verseMarkers || [];
    if (!markers.length) return null;

    const rows = [];
    for (const marker of markers) {
      const verseId = canonicalVerseId(section.book, marker.chapter, marker.verse);
      const text = getCachedTranslationText(verseId);
      if (!text) continue;
      const anchor = marker.anchor || `v-${section.id}-${marker.chapter}-${marker.verse}`.toLowerCase();
      const label = marker.label || `${marker.chapter}:${marker.verse}`;
      rows.push(`<p><span id="${escapeAttr(anchor)}" class="verse-number" data-chapter="${escapeAttr(marker.chapter)}" data-verse="${escapeAttr(marker.verse)}">${escapeHTML(label)}</span> ${escapeHTML(text)}</p>`);
    }

    if (!rows.length) {
      const source = section.source ? `<span class="source-pill">${escapeHTML(section.source)}</span>` : "";
      const tier = section.tier ? `<span class="source-pill">${escapeHTML(section.tier.replace(/^Tier /, "Tier "))}</span>` : "";
      const notice = `<p class="translation-notice">No verse-matched text for ${escapeHTML(state.activeTranslation.toUpperCase())} was found for this passage. Showing the Basileian text.</p>`;
      const paragraphs = section.paragraphs.map(p => `<p>${formatParagraph(p, section)}</p>`).join("");
      return `<section class="passage" id="${escapeAttr(section.id)}" data-section-id="${escapeAttr(section.id)}">
        <h3>${escapeHTML(displayRef(section))}${section.title ? ` — ${escapeHTML(section.title)}` : ""}</h3>
        <div class="passage-meta">${source}${tier}</div>
        <div class="passage-body" data-section-id="${escapeAttr(section.id)}">${notice}${paragraphs}</div>
      </section>`;
    }

    const source = section.source ? `<span class="source-pill">${escapeHTML(section.source)}</span>` : "";
    const transLabel = els.translationSelect?.selectedOptions?.[0]?.textContent || state.activeTranslation.toUpperCase();
    return `<section class="passage translated-passage" id="${escapeAttr(section.id)}" data-section-id="${escapeAttr(section.id)}">
      <h3>${escapeHTML(displayRef(section))}${section.title ? ` — ${escapeHTML(section.title)}` : ""}</h3>
      <div class="passage-meta">${source}<span class="source-pill">${escapeHTML(transLabel)}</span></div>
      <div class="passage-body" data-section-id="${escapeAttr(section.id)}">${rows.join("")}</div>
    </section>`;
  }
  ...
  async function openOriginalLookupFromSelection() {
    ...
    const marker = findNearestVerseMarker(body, range) || (section.verseMarkers || [])[0];
    const verseId = marker ? canonicalVerseId(section.book, marker.chapter, marker.verse) : null;
    const sourceVerse = verseId && window.BASILEIAN_SOURCE_DATA?.verses
      ? window.BASILEIAN_SOURCE_DATA.verses[verseId]
      : null;

    hideSelectionToolbar();

    const wordEntry = await lookupSelectedWord(selected, sourceVerse);
    openOriginalLanguageModal(selected, section, marker, sourceVerse, wordEntry);
  }

  async function lookupSelectedWord(selected, sourceVerse) {
    const firstWord = selected.split(/\s+/)[0] || selected;
    const normalisedSelected = normaliseLookupKey(firstWord);

    let bestToken = null;
    if (sourceVerse?.tokens?.length) {
      bestToken = sourceVerse.tokens.find(token => {
        return normaliseLookupKey(token.source) === normalisedSelected ||
          normaliseLookupKey(token.transliteration) === normalisedSelected;
      }) || null;
    }

    if (window.TranslationsModule && typeof TranslationsModule.getWordEntry === "function") {
      const language = sourceVerse?.language || "grc";
      const query = bestToken?.source || bestToken?.transliteration || firstWord;
      try {
        const entry = await TranslationsModule.getWordEntry(query, language);
        if (entry) return { ...entry, matchedToken: bestToken };
      } catch (error) {
        console.warn("Word lookup failed:", error);
      }
    }

    return bestToken ? { matchedToken: bestToken } : null;
  }

  function openOriginalLanguageModal(selected, section, marker, sourceVerse, wordEntry) {
    const reference = sourceVerse?.reference || (marker ? `${section.book} ${marker.chapter}:${marker.verse}` : displayRef(section));
    const languageLabel = sourceVerse?.language === "cop" ? "Coptic" : sourceVerse?.language === "grc" ? "Greek" : (sourceVerse?.language || "Source language");

    const matched = wordEntry?.matchedToken;
    const number = wordEntry?.youngs || wordEntry?.young || wordEntry?.strongs || wordEntry?.number || "";
    const numberLabel = wordEntry?.youngs || wordEntry?.young ? "Young's / concordance no." : number ? "Strong's / concordance no." : "Concordance no.";

    const lexicalRows = [
      `<dt>Selected text</dt><dd>${escapeHTML(selected)}</dd>`,
      matched ? `<dt>Best source-token match</dt><dd><span class="source-script">${escapeHTML(matched.source)}</span>${matched.transliteration ? ` <span class="muted">(${escapeHTML(matched.transliteration)})</span>` : ""}</dd>` : "",
      wordEntry?.source ? `<dt>Source / lemma</dt><dd><span class="source-script">${escapeHTML(wordEntry.source)}</span></dd>` : "",
      wordEntry?.transliteration ? `<dt>Transliteration</dt><dd>${escapeHTML(wordEntry.transliteration)}</dd>` : "",
      number ? `<dt>${escapeHTML(numberLabel)}</dt><dd>${escapeHTML(number)}</dd>` : "",
      wordEntry?.definition ? `<dt>Definition</dt><dd>${escapeHTML(wordEntry.definition)}</dd>` : ""
    ].filter(Boolean).join("");

    const setupNotice = number || wordEntry?.definition
      ? ""
      : `<p class="translation-notice">This repository currently has verse-level source text and transliteration, but I could not find token-level lemma/concordance data for this selected word. Add <code>lexicons/word-index.json</code> or <code>lexicons/strongs-grc.json</code> plus <code>lexicons/strongs-index.json</code> to enable exact word definitions.</p>`;

    const sourceBlock = sourceVerse
      ? `<h3>${escapeHTML(reference)} · ${escapeHTML(languageLabel)}</h3>
         <div class="source-verse-block">
           <p class="source-script">${escapeHTML(sourceVerse.text || "")}</p>
           ${sourceVerse.transliteration ? `<p class="source-translit">${escapeHTML(sourceVerse.transliteration)}</p>` : ""}
         </div>`
      : `<p class="translation-notice">No source-language verse data was found for ${escapeHTML(reference)}.</p>`;

    openModal("Original-language lookup", `
      <div class="original-lookup">
        ${sourceBlock}
        ${lexicalRows ? `<dl class="lexical-grid">${lexicalRows}</dl>` : ""}
        ${setupNotice}
      </div>
    `, [
      { label: "Close", className: "button", onClick: closeModal }
  ...
  function findNearestVerseMarker(body, range) {
    const markers = Array.from(body.querySelectorAll(".verse-number"));
    let best = null;
    for (const marker of markers) {
      const markerRange = document.createRange();
      markerRange.selectNode(marker);
      const isBeforeOrAtSelection = markerRange.compareBoundaryPoints(Range.START_TO_START, range) <= 0;
      markerRange.detach();
      if (isBeforeOrAtSelection) best = marker;
      else break;
    }
    if (!best) return null;
    return {
      chapter: best.dataset.chapter,
      verse: best.dataset.verse,
      label: best.textContent,
      anchor: best.id
    };
  }
