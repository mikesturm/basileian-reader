...
## Current state of this repository after the translation-fix patch

The app now has the wiring needed for:

- translation switching from the dropdown;
- on-demand loading of public-domain translations;
- fallback fetching from the `source_url` in `translations-index.json` when local `translations/*.json` files are not present;
- an **Original** selection-toolbar action that opens the selected verse's Greek or Coptic source text and transliteration.

The repository still does **not** include complete token-level lemma/concordance data. The `source-data.js` file generated by this patch contains verse-level source text and transliteration derived from the existing `tier*.json` files. To make the lookup show exact Young's/Strong's numbers and definitions for every selected word, add one of these data layers:

1. `lexicons/word-index.json` keyed by normalized selected/source word, with entries such as:
   ```json
   {
     "logos": {
       "source": "λογος",
       ...
       "strongs": "G3056",
       "youngs": "optional-young-number",
       "definition": "word, saying, matter..."
     }
   }
   ```
2. Or `lexicons/strongs-grc.json` plus `lexicons/strongs-index.json`, where the first maps Greek/Coptic surface forms or lemmas to concordance numbers and the second maps those numbers to definitions.

Until those files are added, the Original popup shows the best available verse-level Greek/Coptic text and transliteration and explains that exact word-level concordance data is not yet present.
