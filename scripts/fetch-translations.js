...
const TRANSLATIONS = [
  {
    ...
    url: 'https://raw.githubusercontent.com/openbibledata/kjv/master/kjv.json',
    format: 'openbibledata',
    license: 'Public domain'
  },
  {
    id: 'asv',
    name: 'American Standard Version',
    url: 'https://raw.githubusercontent.com/scrollmapper/bible_databases/master/json/eng-asv.json',
    format: 'scrollmapper',
    license: 'Public domain'
  },
  {
    ...
    url: 'https://raw.githubusercontent.com/scrollmapper/bible_databases/master/json/eng-web.json',
    format: 'scrollmapper',
  ...
  },
  {
    ...
    url: 'https://raw.githubusercontent.com/scrollmapper/bible_databases/master/json/eng-ylt.json',
    format: 'scrollmapper',
    license: 'Public domain'
  },
  {
    ...
    url: 'https://raw.githubusercontent.com/scrollmapper/bible_databases/master/json/eng-dby.json',
    format: 'scrollmapper',
    license: 'Public domain'
  }
];
...
async function parseScrollmapper(url) {
  ...
  // Layout 2: { resultset: { row: [...] } }
  else if (raw.resultset && Array.isArray(raw.resultset.row)) {
    rows = raw.resultset.row.map(r => {
      // Each row has a "field" array: [id, book_num, chapter, verse, text]
      // or is already a plain object.
  ...
  for (const row of rows) {
    // Flat-object layout: { book, chapter, verse, text }
    if (typeof row.book === 'string' && row.chapter !== undefined) {
      result[toVerseId(row.book, row.chapter, row.verse)] = row.text || '';
      continue;
    }
    // Numeric-book layout from resultset/row/field transformation
    if (row.b !== undefined) {
